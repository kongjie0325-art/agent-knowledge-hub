...
> Source: kb/capability/agent-framework.md
...
|------|-------|------|------|------|----------|
| LangChain | 137,836 | Python | 生态最丰富，社区活跃 | 抽象层过多，学习曲线陡 | 快速原型、RAG 应用 |
| AutoGen | 58,477 | Python | 微软出品，多 Agent 对话原生 | 性能开销大 | 多 Agent 协作研究 |
| CrewAI | 52,338 | Python | 上手简单，角色分工直观 | 灵活性不如 LangChain | 团队协作场景 |
| OpenAI Agents | 26,714 | Python | 设计简洁，Handoff 机制优雅 | 生态较新 | 生产级 Agent 应用 |
| MetaGPT | 68,351 | Python | 多角色协作，自动需求分析 | 复杂度高 | 软件工程 Agent |
| Hermes Agent | 170,701 | Python | 工具链完整，技能系统丰富 | 较新，文档待完善 | 通用 Agent 开发 |
...
├── 是 → 需要对话式交互？
│   ├── 是 → AutoGen / CrewAI
│   └── 否 → OpenAI Agents / MetaGPT
└── 否 → 需要丰富工具生态？
    ├── 是 → LangChain / Hermes Agent
    └── 否 → 企业 .NET 环境？
        ├── 是 → Semantic Kernel
        └── 否 → OpenAI Agents
```
